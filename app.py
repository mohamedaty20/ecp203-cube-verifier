...
if input_method == "Manual Entry":
    ...
    st.info("💡 Upload an Excel file. Select columns for each testing age from your file.")
...
if not (cubes_7 or cubes_14 or cubes_28):
    st.warning("⚠️ Please provide cube strength data for at least one testing age (7, 14, or 28 days).")
    st.stop()
...
tabs = st.tabs(["7-Day Stage Compliance", "14-Day Stage Compliance", "28-Day Stage Compliance"])

for idx, (stage_label, tab) in enumerate(zip(["7 Days", "14 Days", "28 Days"], tabs)):
    with tab:
        res = stages_data[stage_label]
        if res is None:
            st.info(f"ℹ️ Insufficient or missing data for {stage_label} testing (minimum 3 cubes required).")
        else:
            ...
            st.write("")
            st.write(f"• **Target Strength Threshold ({int(res['target_ratio']*100)}% of Spec):** `{res['stage_target_fcu']:.2f}` N/mm²")
            ...
            st.write("")
            if res['is_compliant']:
                st.success(f"✅ **PASS ({stage_label}):** Complies with the target requirements of ECP 203 for {stage_label}.")
            else:
...
for stage_key in ["7 Days", "14 Days", "28 Days"]:
    ...
    summary_rows.append({"Metric / Field": f"--- {stage_key} Evaluation ---", "Value": "---"})
    if res:
        summary_rows.extend([
            {"Metric / Field": f"{stage_key} Sample Count (n)", "Value": res['n']},
            {"Metric / Field": f"{stage_key} Target Strength (N/mm²)", "Value": round(res['stage_target_fcu'], 2)},
            {"Metric / Field": f"{stage_key} Mean Strength f_m (N/mm²)", "Value": round(res['mean'], 2)},
            {"Metric / Field": f"{stage_key} Standard Deviation s (N/mm²)", "Value": round(res['s'], 2)},
            {"Metric / Field": f"{stage_key} Margin Factor (k)", "Value": res['k']},
            {"Metric / Field": f"{stage_key} Calculated fcu (N/mm²)", "Value": round(res['fcu_char'], 2)},
            {"Metric / Field": f"{stage_key} Minimum Individual Cube (N/mm²)", "Value": round(res['min'], 2)},
            {"Metric / Field": f"{stage_key} Compliance Verdict", "Value": "PASS" if res['is_compliant'] else "FAIL"}
        ])
    else:
        summary_rows.append({"Metric / Field": f"{stage_key} Data Status", "Value": "No valid data provided"})
...
calculation_notes = [
    ...
    {"Section": "3. Mean Strength (f_m)", "Details": "Calculated as the arithmetic mean per stage: f_m = Sum(f_i) / n."},
    {"Section": "4. Standard Deviation (s)", "Details": "Calculated using sample degrees of freedom: s = sqrt(Sum(f_i - f_m)^2 / (n - 1))."},
    {"Section": "5. Margin Factor (k)", "Details": "If sample size n < 30, k = 1.91. If sample size n >= 30, k = 1.64."},
    {"Section": "6. Characteristic Strength (f_cu)", "Details": "Evaluated per stage as: f_cu,1 = f_m - (k * s) and f_cu,2 = 0.85 * f_m. f_cu = max(f_cu,1, f_cu,2)."},
    {"Section": "7. Acceptance Condition 1", "Details": "The calculated characteristic strength f_cu must meet or exceed the target stage strength."},
    {"Section": "8. Acceptance Condition 2", "Details": "Every individual cube result must be >= 0.85 * (target stage strength)."}
]
...
with st.expander("🔍 Show Step-by-Step ECP 203 Calculation Formulas (All Stages)"):
    for stage_key in ["7 Days", "14 Days", "28 Days"]:
        ...
        st.write(f"👉 **Std. Dev. ($s$)** = `{res['s']:.2f}` N/mm² | **Factor $k$** = `{res['k']}` (Sample $n = {res['n']}$)")
        
        st.latex(r"f_{cu,1} = f_m - (k \times s) \quad | \quad f_{cu,2} = 0.85 \times f_m")
        st.write(f"• $f_{{cu,1}} = {res['mean']:.2f} - ({res['k']} \\times {res['s']:.2f}) =$ **`{res['fcu_calc_1']:.2f}` N/mm²**")
        st.write(f"• $f_{{cu,2}} = 0.85 \\times {res['mean']:.2f} =$ **`{res['fcu_calc_2']:.2f}` N/mm²**")
        st.write(f"👉 **Calculated Stage $f_{{cu}}$** = `{res['fcu_char']:.2f}` N/mm² | **Stage Target:** `{res['stage_target_fcu']:.2f}` N/mm²")
        ...
        st.write(f"• Min Individual Limit: **`{res['min_threshold']:.2f}` N/mm²** | Lowest Cube: **`{res['min']:.2f}` N/mm²**")
